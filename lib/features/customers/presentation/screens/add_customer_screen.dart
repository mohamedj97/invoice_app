        ...
        listener: (context, state) async {
          ...
          if (state.addEditCustomerRequestState == RequestState.error) {
            await getErrorDialogue(
              context: context,
              isUnAuthorized:
              state.addCustomerResponse!.statuscode ==
                  401,
            ...
            body: BlocProvider<GetCustomerTypesCubit>.value(
              ...
              child: BlocConsumer<GetCustomerTypesCubit, GetCustomerTypesState>(
                listener: (context, state) async {
                  ...
                  if (state.getCustomerTypesRequestState ==
                    ...
                    getErrorDialogue(
                      context: context,
                      isUnAuthorized:
                      state.getCustomerTypesResponse!.statuscode ==
                          401,
                ...
                builder: (context, state) {
                  ...
                  if (!hasData) {
                    initialValueCustomerType = state.getCustomerTypesResponse
                        ?.result?.result.customerTypes
                        .firstWhere((element) => element.id==(widget.customerItem!.typeid));

                    initialValueCountry = state.getCustomerTypesResponse
                        ?.result?.result.countries
                        .firstWhere((element) => element.id==(widget.customerItem!.countryid));

                    initialValueGovernment = state.getCustomerTypesResponse
                        ?.result?.result.governates
                        .firstWhere((element) => element.id==(widget.customerItem!.governateid));
                  }
                  countries = state.getCustomerTypesResponse?.result
                      ?.result.countries ??
                      [];

                  governments = state.getCustomerTypesResponse?.result
                      ?.result.governates ??
                      [];

                  customerTypes = state.getCustomerTypesResponse?.result
